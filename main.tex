...
\subsection{Approximating the gradient of logsumexp}
Rather than introducing a surrogate loss, such as equation (\ref{eqn:elbo}),
and losing this built-in baseline,
    we can instead approximate the gradient of logsumexp by using a restriction of logsumexp to
    $\mathcal{Z}'\subseteq\mathcal{Z}$:
\begin{equation}
\log\sum_{z \in \mathcal{Z}'}\exp f(z).
\end{equation}
